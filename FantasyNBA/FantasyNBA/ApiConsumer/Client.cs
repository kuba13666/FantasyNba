...
namespace FantasyNBA.ApiConsumer
{
    public class Client
    {
        ...
        private string UrlPlayerGameLog;
        ...
        public async Task<Playergamelogs> GetPlayerGameLog(int[] players)
        {
            UrlPlayerGameLog = "https://api.mysportsfeeds.com/v1.1/pull/nba/2017-2018-regular/player_gamelogs.json?player=";

            foreach (var player in players)
            {
                UrlPlayerGameLog += "," + player;
            }
            _client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", "a3ViYTEzNjphbndpbDE=");
            _httpResponse = await _client.GetAsync(UrlPlayerGameLog);
            _json = _httpResponse.Content.ReadAsStringAsync().Result;
